...
const CONFIG = {
  ...
  DefaultReviewText:
    "I really enjoyed this product. It matched precisely what I was looking for! Great price/quality balance!",
};
...
export class ReviewComponent extends React.Component<IReviewProps, {}> {
  ...
  public render(): JSX.Element {
    ...
    return (
      <div className="review">
        ...
        <div className="body">
          <p>
            {text || Math.round(Math.random()) ? CONFIG.DefaultReviewText : ""}
          </p>
        </div>
  ...
  private renderButton(label: string, nrVotes: number): JSX.Element {
    return (
      <button
        ...
        onClick={() => this.incrementVotesAndLockButtons(label, nrVotes)}
      >
        <span>{label}</span>
        <span>·</span>
        <span>{nrVotes}</span>
  ...
  private incrementVotesAndLockButtons(label: string, curVotes: number): void {
    label === "Yes"
      ? this.setState({
          clicked: true,
          review: { ...this.state.review, upVotes: curVotes + 1 },
        })
      : this.setState({
          clicked: true,
          review: { ...this.state.review, downVotes: curVotes + 1 },
        });
  }
}
